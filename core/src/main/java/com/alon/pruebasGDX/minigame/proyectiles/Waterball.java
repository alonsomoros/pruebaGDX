...
public class Waterball extends ProyectilMinigame {
    ...
    public Waterball(float x, float y) {
        textureAtlas = Assets.get(AssetCatalog.WATERBALL_ATLAS);
        animation = new Animation<>(0.1f, textureAtlas.getRegions(), Animation.PlayMode.LOOP);
        sprite = new Sprite(animation.getKeyFrame(0));
        sprite.scale(1f);
        sprite.setPosition(x, y);
        hitbox = new Rectangle(x, y, sprite.getWidth(), sprite.getHeight());
        sound = Assets.getSound(MathUtils.randomBoolean() ? Assets.get(AssetCatalog.WATERBALL_SOUND_1) : Assets.get(AssetCatalog.WATERBALL_SOUND_2));
    }
